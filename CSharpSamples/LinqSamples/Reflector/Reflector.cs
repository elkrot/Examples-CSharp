...
// Компонент Reflector создает HTML-документ с общим API
// для заданной сборки. Эта задача выполняется в 2 этапа. На первом этапе XML- 
// документ создается в памяти (см. Emit*()). Документ собирает 
// соответствующие метаданные сборки. На втором этапе HTML-
// документ извлекается из  XML-документа (см. Extract*()). Следует отметить, 
// что оба этапа обрабатывают одинаковые данные в различных представлениях с помощью 
// тех же самых конструкций запросов.
...
{   
    public class Reflector
    {
        ...
        static string ExtractName(XElement element)
        {
            ...
            if (i > 0) name = name.Substring(0, i); // исправить универсальное имя
            return name;
        }                        
