...
namespace DataAnalysisExcel
{
    ...
    {
        // Этот метод переопределен в указанном далее классе OperationsData.
    ...
    internal class OperationsData : OperationsBaseData
    {
            ...
            foreach (OperationsBaseData.SalesRow row in this.Sales)
            {
                if (lastInventory.ContainsKey(row.Flavor))
                {
                    ...
                    if (!row.IsInventoryNull())
                    {
                        ...
                        // Обновите оценки и рекомендации для строк последнего дня.
                        // Оценки и рекомендации могут отсутствовать в данных, или
                        // рекомендация может быть на неправильном языке.
                        if (row.Date == maxDate)
                        {
        ...
        protected override void OnSalesInventoryChanged(DataColumnChangeEventArgs e)
        {
            ...
            // Изменение запаса означает изменение продаж на текущий и следующий день.
            if (row.RowState == DataRowState.Modified || row.RowState == DataRowState.Added)
            {
                ...
                // Теперь, когда вычислены все соответствующие столбцы продаж,
                // можно оценить запас.
                EstimateInventory(row, sellingSpeed);
            }
        }
